...
class ClickerState:
    ...
    def __str__(self):
        ...
        """
        return_string = "Time: " + str(self._time)\
...
def simulate_clicker(build_info, duration, strategy):
    ...
    while game_state.get_time() <= duration:
        ...
        next_item = strategy(game_state.get_cookies(),
                                    game_state.get_cps(),
                                    game_state.get_history(),
                                    time_left, build_object)
        if next_item == None:
            break
        elif game_state.time_until(build_object.get_cost(next_item)) > time_left:
            break
        else:
            game_state.wait(game_state.time_until(build_object.get_cost(next_item)))
            game_state.buy_item(next_item,
                                build_object.get_cost(next_item),
                                build_object.get_cps(next_item))
            build_object.update_item(next_item)
...
def item_price_dict(build_info):
    ...
    prices = map(build_info.get_cost, all_items)
    items_prices = zip(all_items, prices)
    items_prices_mut = dict((item_name, price) for (item_name, price) in items_prices)
    return items_prices_mut
